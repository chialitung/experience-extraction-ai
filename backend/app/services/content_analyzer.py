...
class ContentAnalyzer:
    ...
    def detect_off_topic(self, answer: str, theme: str, current_step: str) -> Dict[str, Any]:
        """偏离主题检测

        判断用户回答是否偏离了当前访谈主题和步骤目标。

        Returns:
            dict: {is_off_topic: bool, confidence: float, reason: str}
        """
        if not answer:
            return {"is_off_topic": False, "confidence": 0.0, "reason": "空回答，无法判断。"}
        ...
        # 2. 主题关键词缺失度（简单实现：主题词在回答中出现频率）
        theme_keywords = self._extract_keywords(theme)
        matched_keywords = [k for k in theme_keywords if k in text]
        if theme_keywords:
            theme_match_ratio = len(matched_keywords) / len(theme_keywords)
            if theme_match_ratio < 0.3:
                score += 0.20
                signals.append(f"主题关键词匹配度低（{theme_match_ratio:.0%}），可能偏离主题")

        # 3. 步骤相关度（根据当前步骤检测应有内容）
        step_relevance = self._check_step_relevance(text, current_step)
        if step_relevance < 0.3:
            score += 0.20
            signals.append(f"与当前步骤'{current_step}'的相关度低")
    ...
    def _extract_keywords(self, theme: str) -> List[str]:
        """从主题中提取关键词（简单分词）"""
        # 去除常见虚词，保留实词
        stop_words = {"的", "了", "和", "与", "在", "是", "为", "对", "如何", "怎样", "怎么", "一个", "经验", "萃取"}
        words = []
    ...
    def _check_step_relevance(self, text: str, current_step: str) -> float:
        """检查回答与当前步骤的相关度（0.0 ~ 1.0）"""
        step_keywords = {
            "event_review": ["案例", "客户", "当时", "情况", "背景", "事件", "经历", "有一次"],
            "framework_build": ["框架", "步骤", "流程", "方法", "原则", "核心", "总结", "归纳"],
            "detail_mining": ["细节", "具体", "话术", "说", "问", "怎么", "动作", "操作"],
            "obstacle_identify": ["困难", "问题", "错误", "踩坑", "失败", "注意", "避免", "风险"],
            "tool_extract": ["工具", "模板", "检查表", "话术", "卡片", "文档", "清单", "口诀"],
            "confirmation": ["确认", "对吗", "是否", "准确", "补充", "纠正"],
        }

        keywords = step_keywords.get(current_step, [])
        if not keywords:
            return 0.5
    ...
    def full_analysis(self, answer: str, theme: str, current_step: str,
                      ...
                      blueprint: Optional[Dict[str, Any]] = None) -> AnswerAnalysis:
        """完整分析：一次调用获取所有分析结果"""
        depth_result = self.analyze_answer_depth(answer)
        off_topic_result = self.detect_off_topic(answer, theme, current_step)
